...
a function is typdef'd below

can add, subtract, multiply and divide functions

can create new named functions (stored in User because cannot directly modify functions as it's a metatype)
...
the library must be imported in one of two ways: i) local name_you_want=require "alg.functions" () --this will bring in the library and user defined functions will be stored in global table USER (functions can also be )
or ii) local name_you_want_for_default, name_you_want_for_your_own_functions=require "alg.functions"()

user defined functions can be defined in the following ways:
...
function User_mt.__call(self, t)
  ...
  self.defined_string=self.defined_string..', [USER] '..name
  self.defined_table[#self.defined_table+1]=name
  return self[name]
end

function functions.__add(f1, f2)
  return functions.New_Function(function(x) return f1.func(x)+f2.func(x) end,function(x) return f1.derivative(x)+f2.derivative(x) end)
end
...
function functions.__mul(f1, f2)
  return functions.New_Function(function(x) return f1.func(x)*f2.func(x) end,function(x) return f1.derivative(x)*f2.func(x)+f1.func(x)*f2.derivative(x) end)
end

function functions.__div(f1, f2)
  return functions.New_Function(function(x) return f1.func(x)/f2.func(x) end,function(x) return (f1.derivative(x)*f2.func(x)-f1.func(x)*f2.derivative(x))/f2.func(x)^2 end)
end

function functions.List_Functions(o)
  --function to tell what functions have been declared.
  --returns table by default
  assert(o==nil or o=='string' or o=='table', "List_Functions Error: Option is either nil, 'string' or 'table'.")
  local o=o or 'table'
